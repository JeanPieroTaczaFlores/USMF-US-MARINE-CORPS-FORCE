...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "create") {
      ...
      const { data: created, error: createError } = await admin.auth.admin.createUser({ email: body.email, password: body.password, email_confirm: true, user_metadata: { nombre: body.nombre, usuario_roblox: body.usuario_roblox } });
    ...
    if (body.action === "update") {
      ...
      if (body.email && body.email !== target.email) {
        const { error: emailError } = await admin.auth.admin.updateUserById(target.id, { email: body.email, email_confirm: true });
